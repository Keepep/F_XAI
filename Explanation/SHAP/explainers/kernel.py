...
class KernelExplainer(Explainer):
    ...
    def __init__(self, model, data, link=IdentityLink(), **kwargs):
        ...
        self.data = convert_to_data(data, keep_index=self.keep_index) #train data
        model_null = match_model_to_data(self.model, self.data) #result probability from train data
        ...
        # init our parameters
        self.N = self.data.data.shape[0] #the number of instances
        self.P = self.data.data.shape[1] #teh number of features
        ...
        self.fnull = np.sum((model_null.T * self.data.weights).T, 0) #average result probability from train dataset
        ...
        if len(self.fnull.shape) == 0:
            ...
            self.D = self.fnull.shape[0] #the number of classes
    ...
    def shap_values(self, X, **kwargs):
        ...
        if str(type(X)).endswith("pandas.core.series.Series'>"):
            ...
            X = X.values #extract test data values from dataframe of that
        ...
        # explain the whole dataset
            ...
            for i in tqdm(range(X.shape[0]), disable=kwargs.get("silent", False)):
                data = X[i:i + 1, :] #select one data from dataset
                if self.keep_index:
    ...
    def explain(self, incoming_instance, **kwargs):
        ...
        instance = convert_to_instance(incoming_instance) #insatnce.x be data
        ...
        if self.keep_index:
            ...
            model_out = self.model.f(instance.x) #result probability of selected data
        if isinstance(model_out, (pd.DataFrame, pd.Series)):
            model_out = model_out.values
        self.fx = model_out[0]  #result probability of selected data
	    ...
	    #adjust the number of samples
            ...
            num_subset_sizes = np.int(np.ceil((self.M - 1) / 2.0)) #define subset size
            num_paired_subset_sizes = np.int(np.floor((self.M - 1) / 2.0))
            weight_vector = np.array([(self.M - 1.0) / (i * (self.M - i)) for i in range(1, num_subset_sizes + 1)]) #(M-1)/(M choose |z'|)
            weight_vector[:num_paired_subset_sizes] *= 2 #multifly 2
            ...
            num_full_subsets = 0
            num_samples_left = self.nsamples #indicate how many samples left
            ...
            for subset_size in range(1, num_subset_sizes + 1):
                ...
                if subset_size <= num_paired_subset_sizes: nsubsets *= 2 #Why???
            ...
            if num_full_subsets != num_subset_sizes:
                ...
                while samples_left > 0:
                    ...
                    if mask_tuple not in used_masks:
                        ...
                        samples_left -= 1 # samples left is reduced
                    ...
                    if samples_left > 0 and subset_size <= num_paired_subset_sizes:
                        mask[:] = np.abs(mask - 1) #change 1 to 0 and vice versa
            ...
            phi = np.zeros((self.data.groups_size, self.D))
            phi_var = np.zeros((self.data.groups_size, self.D))
            for d in range(self.D): #D is channel number
    ...
    def addsample(self, x, m, w):
        ...
        if isinstance(self.varyingFeatureGroups, (list,)):
            ...
            mask = m == 1.0 # if m==1.0, value of its index be True
            groups = self.varyingFeatureGroups[mask] #insert index corresponding with True value into groups
            if len(groups.shape) == 2:
                ...
                # choose only the value of the group index portion of all x's indexes and insert them into train data
    ...
    def run(self):
        num_to_run = self.nsamplesAdded * self.N - self.nsamplesRun * self.N #number to be runned
        ...
        modelOut = self.model.f(data) #result probability of permuted data(self.synth_data)
        if isinstance(modelOut, (pd.DataFrame, pd.Series)):
            modelOut = modelOut.values
        self.y[self.nsamplesRun * self.N:self.nsamplesAdded * self.N, :] = np.reshape(modelOut, (num_to_run, self.D))

        # find the expected value of each output
        # so find out that which permutation can most effect
    ...
    def solve(self, fraction_evaluated, dim):
        t1=self.linkfv(self.ey[:, dim])
        t2=self.link.f(self.fnull[dim])
        eyAdj = self.linkfv(self.ey[:, dim]) - self.link.f(self.fnull[dim]) #link.f is np.log(x/(1-x))
        s = np.sum(self.maskMatrix, 1) #the number of masked value
